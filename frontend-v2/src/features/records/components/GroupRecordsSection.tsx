...
interface GroupRecordsSectionProps {
...
export function GroupRecordsSection({ records, projectNamesById, onActionError, actions }: GroupRecordsSectionProps) {
  ...
  return (
    ...
    >
      {records.length === 0 ? (
        <EmptyState
          title="Sem registros neste grupo"
          description="Decisões, pendências e observações dos projetos aparecerão aqui em lista única."
          className="py-8"
        />
      ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-text-tertiary">
                      Projeto: {projectNamesById[record.projetoId] || 'Projeto sem nome'} · Autor: {record.autorNome}
                    </p>
                  </div>
