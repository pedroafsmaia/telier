...
export const GlobalTimerBar: React.FC = () => {
  ...
  let badge: React.ReactNode = (
    <span className="inline-flex h-7 items-center rounded-full border border-border-primary bg-surface-primary px-3 text-xs font-medium text-text-secondary">
      Sem timer ativo
    </span>
  );
  ...
  if (isLoading && !hasAnyActiveSession) {
    badge = (
      <span className="inline-flex h-7 items-center gap-1.5 rounded-full border border-border-primary bg-surface-primary px-3 text-xs font-medium text-text-secondary">
    ...
    badge = (
      <span className="inline-flex h-7 items-center gap-1.5 rounded-full border border-alert-subtle bg-alert-subtle/20 px-3 text-xs font-medium text-alert">
        <AlertTriangle className="h-3.5 w-3.5" />
        Erro de rede
      </span>
    );
    statusDescription = 'Nao foi possivel atualizar os timers ativos.';
    ...
    statusDescription = `Seu timer esta ativo em ${currentUserSession.projetoNome}: ${currentUserSession.tarefaNome}.`;
    if (isError) {
      statusDescription = `${statusDescription} Atualizacao de rede instavel.`;
      statusClassName = 'text-alert';
    }
